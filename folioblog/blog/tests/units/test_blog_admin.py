...
class BlogCategoryChooserViewSetTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.site = Site.objects.get(is_default_site=True)
        cls.admin = UserFactory(is_superuser=True)

        cls.cat1 = BlogCategoryFactory(site=cls.site)
        cls.cat2 = BlogCategoryFactory(site=SiteFactory())

        cls.url = reverse("blogcategory_chooser:choose_results")
...
class BlogCategorySnippetViewTestCase(TestCase):
    @classmethod
    def setUpTestData(cls):
        cls.site = Site.objects.get(is_default_site=True)
        cls.admin = UserFactory(is_superuser=True)
        ...
        cls.url = reverse("wagtailsnippets_blog_blogcategory:list")
